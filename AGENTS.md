...
- A heading `### WBU-N: Title`, then a field block, then free prose context.
...
- Fields: `status` (`todo` / `in-progress` / `draft` / `done`), `priority`
  ...
  (other `WBU-N` ids), a `touches:` list where the rule below applies,
...
- Completed items move **verbatim** (number, title, field block, prose, with
  ...
  keeps WBU-N references resolvable. CHANGELOG.md remains the permanent record
...
- Work discovered mid-implementation gets its own item immediately, noting
  `discovered-from: WBU-N` in its prose, plus a `blocked-by` link if it blocks
  anything.
...
landed, including that row's ARCHITECTURE/AGENTS docs) or explicitly waived
(`unaffected: <repo> (<why>)`). A breaking change is named as such in each
implementing package's CHANGELOG.

The per-Space archive layout each bundle carries is a separate contract, owned
by `@interop/space-archive`. See that package's AGENTS.md for its own parties
table.
